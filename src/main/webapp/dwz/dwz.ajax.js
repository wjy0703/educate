...
function navTabAjaxDone(json){
    if(json.statusCode == DWZ.statusCode.nothing)
    	return ;
	DWZ.ajaxDone(json);
	if (json.statusCode == DWZ.statusCode.ok){
		if (json.navTabId){ //把指定navTab页面标记为需要“重新载入”。注意navTabId不能是当前navTab页面的
			var navTabIds = new Array(); //定义一数组
			navTabIds = json.navTabId.split(","); //字符分割   
			for (var i=0;i<navTabIds.length ;i++ ){
				navTab.reloadFlag(navTabIds[i]);
			}
		} else { //重新载入当前navTab页面
 ...
 * @param {Object} form
 */
function divValidateSearch(form, rel){
	var $form = $(form);
	if (!$form.validDiv(form)) {
		return false;
	}
	if (form[DWZ.pageInfo.pageNum]) form[DWZ.pageInfo.pageNum].value = 1;
	if (rel) {
