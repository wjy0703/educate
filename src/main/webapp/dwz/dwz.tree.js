 ...
 (function($){
 	$.extend($.fn, {
						...
						if(checkFn && $.isFunction(checkFn)) {
							$("div.ckbox", $this).each(function(){
								...
								ckbox.click(function(){
									...
									checkFn({checked:itemsNewRes, items:itemsNew});		
									itemsNewLen = 0;		
									itemsNewRes = true;													
								});
							});
		...
		_checkParent:function(){
			...
			itemsNew[itemsNewLen] = {name:$("input", parent).attr("name"), value:$("input", parent).val(), text:$("input", parent).attr("text")};
			itemsNewLen += 1;
			parent._checkParent();
		}
	});
