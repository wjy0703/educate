...
(function($){
	...
	$.extend({
		...
		idBringBack:function(args){
			$.idBringBackSuggest(args);
			$.pdialog.closeCurrent();
		},
		idBringBackSuggest: function(args){
			var $box = _lookup['$target'].parents(".unitBox:first");
			if(args['clazz']){
				$box.find("." + args['clazz']).each(function(){
					var $this = $(this); 
					$this.val(args[$this.attr("id")]);
				});				 
			}else{
				for (var key in args) {
					if(key != 'clazz'){
						var $input = $box.find("#" + key);
						$input.val(args[key]);
					}
				}
			}
		...
		multLookup: function(){
			return this.each(function(){
				...
				$this.click(function(event){
					...
					//选择带回为null时，不让带回，为了处理取消1个选择后，不让带回的问题
