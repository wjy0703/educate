...
var alertMsg = {
	...
	errorInvalidate:function(title,msg,options){
		var op = {okName:$.regional.alertMsg.butMsg.ok, okCall:null};
		$.extend(op, options);
		var button = {name:op.okName, call: op.okCall, keyCode:DWZ.keyCode.ENTER};
		button.firstElement = options.firstElement;
		this._openInvalidate(title,msg, button);
	},
	_openInvalidate: function(title, msg, button){
		$(this._boxId).remove();
		var butsHtml = "";
		if (button) {
				var sRel = button.call ? "callback" : "";
				butsHtml += DWZ.frag["alertButFrag"].replace("#butMsg#", button.name).replace("#callback#", sRel);
		}
		var boxHtml = DWZ.frag["alertBoxFrag"].replace("#type#", "error").replace("#title#", title).replace("#message#", msg).replace("#butFragment#", butsHtml);
		$(boxHtml).appendTo("body").css({top:-$(this._boxId).height()+"px"}).animate({top:"0px"}, 500);
		...
		var jCallButs = jButs.filter("[rel=callback]");
		var jDoc = $(document);
		var $firstElement = button.firstElement;
//		if (button.call) jCallButs.eq(0).click(button.call);

		if (button.call) jCallButs.eq(0).bind('click',{firstElement:$firstElement},button.call);
		if (button.keyCode == DWZ.keyCode.ENTER) {
			jDoc.bind("keydown",{target:jButs.eq(0)}, this._keydownOk);
		}
		if (button.keyCode == DWZ.keyCode.ESC) {
			jDoc.bind("keydown",{target:jButs.eq(0)}, this._keydownEsc);
		}
	}
