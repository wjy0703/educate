/* 验证如出现问题，则打开此方法，同时去掉下面的同名方法
 */
function invalidateMessage(form, validator) {
		...
		$invalidElements.each(function() {
			...
			if(!title){
				if (this.type == 'select-one') {
					title = $this.parents('div.combox').prev('label').html();
					if (title == null)
						title = $this.parents('div.combox').parent('dd').prev('dt')
								.html();
			...
			if(!title){
				title = $this.parents('td').prev('td').html();
			}
			var errMessage = eval('errors["' + name + '"]');
			message += '<font color="blue">' + title
					+ '</font> <font color="red"> ' + errMessage
					+ '</font><br>';
		});
		var title = '请改正以下' + numberErrors +"个错误后提交<br>";
		alertMsg.error(message);
		setTimeout(function(){			
			$('#alertMsgBox').find('div.alertInner>h1').html(title);
		},150);
	}
}

/*function focusError(elem){
	var $firstElement = elem.data.firstElement;
	if($firstElement.is('input')){
		$firstElement.focus();
		$firstElement.removeClass('focus valid').addClass('error');
	}else if($firstElement.is('select')){
		$firstElement.prev('a').focus();
	}
}
...
function invalidateMessage(form, validator) {
	...
	if ($invalidElements.length > 0) {
		...
		var options={
				okCall:focusError,
				firstElement:$invalidElements.eq(0)
		};
		var title = '请改正以下' + numberErrors +"个错误后提交<br>";
		alertMsg.errorInvalidate(title, message, options);
	}
}*/
...
function initUI(_box){
	...
	$("form.required-validate", $p).each(function(){
		...
		$form.validate({
			...
			errorElement: "span",
			ignore:".ignore",
			invalidHandler:invalidateMessage
		});
