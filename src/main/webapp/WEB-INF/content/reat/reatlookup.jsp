...
<div class="pageHeader">
	<form rel="pagerForm" method="post"
		action="${ctx }/businessinfo/findreat?reatid=${reatid}"
				...
				<ul>
					...
					<li><div class="button">
							<div class="buttonContent">
								<button type="button" multLookup="orgId" warn="请选择套餐">选择带回</button>
							</div>
						</div></li>
				</ul>
			</div>
...
<div class="pageContent">
	...
	<table class="table" layoutH="128" targetType="dialog" width="100%">
		...
		<tbody>
			<c:forEach items="${result}" var="reat" varStatus="st">
				<tr>
					<td>
					<input type="radio" name="orgId"
						value="{id:'${reat.id }', name:'${reat.reatname}'}"
						<c:if test="${reat.flag=='Y'}">checked</c:if> /></td>
					<td>${reat.reatname}</td>
					<td><sen:vtoName coding="cyckes" value="${reat.cycke}"/></td>
					<td>${reat.price}</td>
				</tr>
			</c:forEach>
		</tbody>
